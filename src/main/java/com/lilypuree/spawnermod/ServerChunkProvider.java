...
public class ServerChunkProvider extends AbstractChunkProvider {
    ...
    @Nullable
    private ChunkHolder getChunkHolder(long chunkPosIn) {
    ...
    public Chunk getChunkNow(int chunkX, int chunkZ) {
        if (Thread.currentThread() != this.mainThread) {
            ...
            ChunkHolder chunkholder = this.getChunkHolder(i);
            if (chunkholder == null) {
                return null;
            } else {
    ...
    private CompletableFuture<Either<IChunk, ChunkHolder.IChunkLoadingError>> func_217233_c(int chunkX, int chunkZ, ChunkStatus requiredStatus, boolean load) {
        ...
        if (load) {
            ...
            if (this.func_217224_a(chunkholder, j)) {
                ...
                this.func_217235_l();
                chunkholder = this.getChunkHolder(i);
    ...
    public boolean chunkExists(int x, int z) {
        ChunkHolder chunkholder = this.getChunkHolder((new ChunkPos(x, z)).asLong());
    ...
    private void tickChunks() {
        ...
        if (!isDebug) {
            ...
            int spawningChunksCount = this.ticketManager.getSpawningChunksCount();
            WorldEntitySpawner.EntityDensityManager densityManager = WorldEntitySpawner.func_234964_a_(spawningChunksCount, this.world.func_241136_z_(), this::func_241098_a_);
            this.entityDensityManager = densityManager;
            this.world.getProfiler().endSection();
                ...
                if (optional.isPresent()) {
                    ...
                    if (optional1.isPresent()) {
                        ...
                        if (!this.chunkManager.isOutsideSpawningRadius(chunkpos)) {
                            ...
                            if (doMobSpawning && (this.spawnHostiles || this.spawnPassives) && this.world.getWorldBorder().contains(chunk.getPos())) {
                                WorldEntitySpawner.spawnMobs(this.world, chunk, densityManager, this.spawnPassives, this.spawnHostiles, flag2);
                            }

                            this.world.tickEnvironment(chunk, randomTickSpeed);
                        }
                    }
                }
            });
    ...
    private void func_241098_a_(long chunkPos, Consumer<Chunk> p_241098_3_) {
    ...
    public void markBlockChanged(BlockPos pos) {
        int i = pos.getX() >> 4;
        int j = pos.getZ() >> 4;
        ChunkHolder chunkholder = this.getChunkHolder(ChunkPos.asLong(i, j));
        if (chunkholder != null) {
    ...
    public void markLightChanged(LightType type, SectionPos pos) {
        this.executor.execute(() -> {
            ChunkHolder chunkholder = this.getChunkHolder(pos.asChunkPos().asLong());
            if (chunkholder != null) {
