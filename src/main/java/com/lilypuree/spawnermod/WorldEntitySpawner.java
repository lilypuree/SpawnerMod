...
public final class WorldEntitySpawner {
    ...
    private static final EntityClassification[] spawnClassifications = Stream.of(EntityClassification.values()).filter((classification) -> {
        return classification != EntityClassification.MISC;
    }).toArray((values) -> {
        return new EntityClassification[values];
    });

    public static WorldEntitySpawner.EntityDensityManager func_234964_a_(int p_234964_0_, Iterable<Entity> entities, WorldEntitySpawner.IInitialDensityAdder initialDensityAdder) {
        MobDensityTracker mobdensitytracker = new MobDensityTracker();
        Object2IntOpenHashMap<EntityClassification> entityClassifications = new Object2IntOpenHashMap<>();
        ...
        while(true) {
            ...
            do {
                if (!iterator.hasNext()) {
                    return new WorldEntitySpawner.EntityDensityManager(p_234964_0_, entityClassifications, mobdensitytracker);
                }
            ...
            if (entityclassification != EntityClassification.MISC) {
                ...
                initialDensityAdder.query(i, (chunk) -> {
                    MobSpawnInfo.SpawnCosts spawnCosts = getBiome(blockpos, chunk).getMobSpawnInfo().getSpawnCost(entity_f.getType());
                    if (spawnCosts != null) {
                        mobdensitytracker.addTracker(entity_f.getPosition(), spawnCosts.getEntitySpawnCost());
                    }

                    entityClassifications.addTo(entityclassification, 1);
                });
            }
        }
    }

    private static Biome getBiome(BlockPos pos, IChunk chunk) {
        return DefaultBiomeMagnifier.INSTANCE.getBiome(0L, pos.getX(), pos.getY(), pos.getZ(), chunk.getBiomes());
    }

    public static void spawnMobs(ServerWorld world, Chunk chunk, WorldEntitySpawner.EntityDensityManager densityManager, boolean spawnPassives, boolean spawnHostiles, boolean p_234979_5_) {
        world.getProfiler().startSection("spawner");

        for(EntityClassification entityclassification : spawnClassifications) {
            if ((spawnPassives || !entityclassification.getPeacefulCreature()) && (spawnHostiles || entityclassification.getPeacefulCreature()) && (p_234979_5_ || !entityclassification.getAnimal()) && densityManager.func_234991_a_(entityclassification)) {
                func_234967_a_(entityclassification, world, chunk, (p_234969_1_, p_234969_2_, p_234969_3_) -> {
                    return densityManager.func_234989_a_(p_234969_1_, p_234969_2_, p_234969_3_);
                }, (p_234970_1_, p_234970_2_) -> {
                    densityManager.func_234990_a_(p_234970_1_, p_234970_2_);
                });
            }
        }
    ...
    public static void func_234967_a_(EntityClassification classification, ServerWorld world, Chunk chunk, WorldEntitySpawner.IDensityCheck densityCheck, WorldEntitySpawner.IOnSpawnDensityAdder onSpawnDensityAdder) {
        BlockPos blockpos = getRandomHeight(world, chunk);
        if (blockpos.getY() >= 1) {
            func_234966_a_(classification, world, chunk, blockpos, densityCheck, onSpawnDensityAdder);
        }
    }

    public static void func_234966_a_(EntityClassification classification, ServerWorld world, IChunk chunk, BlockPos pos, WorldEntitySpawner.IDensityCheck densityCheck, WorldEntitySpawner.IOnSpawnDensityAdder onSpawnDensityAdder) {
        ...
        if (!blockstate.isNormalCube(chunk, pos)) {
            ...
            for(int k = 0; k < 3; ++k) {
                ...
                int k1 = MathHelper.ceil(world.rand.nextFloat() * 4.0F);
                int l1 = 0;

                for(int i2 = 0; i2 < k1; ++i2) {
                    l += world.rand.nextInt(6) - world.rand.nextInt(6);
                    i1 += world.rand.nextInt(6) - world.rand.nextInt(6);
                    mutable.setPos(l, y, i1);
                    double d0 = (double)l + 0.5D;
                    double d1 = (double)i1 + 0.5D;
                    PlayerEntity playerentity = world.getClosestPlayer(d0, (double)y, d1, -1.0D, false);
                    if (playerentity != null) {
                        double d2 = playerentity.getDistanceSq(d0, (double)y, d1);
                        if (func_234978_a_(world, chunk, mutable, d2)) {
                            if (spawners == null) {
                                spawners = func_234977_a_(world, structuremanager, chunkgenerator, classification, world.rand, mutable);
                                if (spawners == null) {
                                    break;
                                }

                                k1 = spawners.minCount + world.rand.nextInt(1 + spawners.maxCount - spawners.minCount);
                            }

                            if (func_234975_a_(world, classification, structuremanager, chunkgenerator, spawners, mutable, d2) && densityCheck.test(spawners.type, mutable, chunk)) {
                                MobEntity mobentity = func_234973_a_(world, spawners.type);
                                if (mobentity == null) {
                                    return;
                                }

                                mobentity.setLocationAndAngles(d0, (double)y, d1, world.rand.nextFloat() * 360.0F, 0.0F);
                                int canSpawn = net.minecraftforge.common.ForgeHooks.canEntitySpawn(mobentity, world, d0, y, d1, null, SpawnReason.NATURAL);
                                if (canSpawn != -1 && (canSpawn == 1 || func_234974_a_(world, mobentity, d2))) {
                                    if (!net.minecraftforge.event.ForgeEventFactory.doSpecialSpawn(mobentity, world, (float)d0, (float)y, (float)d1, null, SpawnReason.NATURAL))
                                    ...
                                    onSpawnDensityAdder.run(mobentity, chunk);
    ...
    public static class EntityDensityManager {
        ...
        private final Object2IntOpenHashMap<EntityClassification> entityClassifications;
        private final MobDensityTracker densityTracker;
        ...
        private EntityDensityManager(int p_i231621_1_, Object2IntOpenHashMap<EntityClassification> entityClassifications, MobDensityTracker densityTracker) {
            this.field_234981_a_ = p_i231621_1_;
            this.entityClassifications = entityClassifications;
            this.densityTracker = densityTracker;
            this.field_234984_d_ = Object2IntMaps.unmodifiable(entityClassifications);
        }

        private boolean func_234989_a_(EntityType<?> type, BlockPos pos, IChunk chunk) {
            this.field_234985_e_ = pos;
            this.field_234986_f_ = type;
            MobSpawnInfo.SpawnCosts spawnCosts = WorldEntitySpawner.getBiome(pos, chunk).getMobSpawnInfo().getSpawnCost(type);
            if (spawnCosts == null) {
                this.entitySpawnCost = 0.0D;
                return true;
            } else {
                double curSpawnCost = spawnCosts.getEntitySpawnCost();
                this.entitySpawnCost = curSpawnCost;
                double d1 = this.densityTracker.func_234999_b_(pos, curSpawnCost);
                return d1 <= spawnCosts.getMaxSpawnCost();
            }
        }

        private void func_234990_a_(MobEntity entity, IChunk chunk) {
            EntityType<?> entitytype = entity.getType();
            BlockPos blockpos = entity.getPosition();
            double spawnCost;
            if (blockpos.equals(this.field_234985_e_) && entitytype == this.field_234986_f_) {
                spawnCost = this.entitySpawnCost;
            } else {
                MobSpawnInfo.SpawnCosts spawnCosts = WorldEntitySpawner.getBiome(blockpos, chunk).getMobSpawnInfo().getSpawnCost(entitytype);
                if (spawnCosts != null) {
                    spawnCost = spawnCosts.getEntitySpawnCost();
                } else {
                    spawnCost = 0.0D;
                }
            }

            this.densityTracker.addTracker(blockpos, spawnCost);
            this.entityClassifications.addTo(entitytype.getClassification(), 1);
        }
        ...
        private boolean func_234991_a_(EntityClassification entityClassification) {
            int i = entityClassification.getMaxNumberOfCreature() * this.field_234981_a_ / WorldEntitySpawner.field_234960_b_;
            return this.entityClassifications.getInt(entityClassification) < i;
        }
    }
